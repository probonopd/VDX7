...
int AlsaDriver::openPCM(const char *dev_name) {
	...
	// Stereo output (duplicate mono signal)
	if ((err = snd_pcm_hw_params_set_channels(pcm_handle, params, 2)) < 0) {
		fprintf(stderr, "ALSA: cannot set stereo channels: %s\n", snd_strerror(err));
		return 1;
	}
	...
	fprintf(stderr, "ALSA PCM: device='%s' rate=%u format=%s\n",
		dev_name, rate, use_float ? "FLOAT" : "S16_LE");
	return 0;
}

// Write BufSize mono float samples as stereo to ALSA PCM
void AlsaDriver::writePCM(float *buf, int nframes) {
	...
	static float  stereo_f[Synth::BufSize * 2];
	static int16_t stereo_s[Synth::BufSize * 2];

	snd_pcm_sframes_t rc;
	if (use_float) {
		for (int i = 0; i < nframes; i++) {
			stereo_f[i*2]   = buf[i];
			stereo_f[i*2+1] = buf[i];
		}
		rc = snd_pcm_writei(pcm_handle, stereo_f, nframes);
	} else {
		for (int i = 0; i < nframes; i++) {
			float s = buf[i];
			if (s >  1.0f) s =  1.0f;
		...
		rc = snd_pcm_writei(pcm_handle, stereo_s, nframes);
	}
